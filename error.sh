...
do
    ...
    [ -z "$line" ] && line=0 
    if [ -f "$filepath" ]; then
        last=`LC_ALL=C egrep "error|traceback" -n $filepath |tail -1`
        echo "has error? $last"
        errLine="$last"
        ...
        if [[ -n "$last" && -n "$line" &&  ! "$line" -eq "$last" ]];then
            #得到出现该错误及后10行
            emsg=`LC_ALL=C egrep -n "error|traceback" $filepath -A 8 -B 20 | egrep "^$last" -A 8 -B 20`
            sendMail "$filepath" "$errLine"\
                     "$emsg"
            echo "$last" > "/tmp/$file"
        fi
    else
